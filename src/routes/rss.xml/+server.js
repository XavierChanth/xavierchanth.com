...
import { BLOG_AUTHOR, BLOG_DESCRIPTION, BLOG_TITLE, BLOG_URL } from '$lib/metadata';
...
async function getRssXml() {
	...
	const root = create({ version: '1.0', encoding: 'utf-8' })
		...
		.ele('channel')
		.ele('atom:link', { rel: 'self', type: 'application/rss+xml', href: rssUrl })
		.up()
		.ele('title')
		.txt(BLOG_TITLE)
		.up()
	...
	for (const post of posts) {
		...
		root
			...
			.up()
			.ele('content:encoded')
			.txt((await getHtmlForPost(post.slug)).replaceAll('&', '&amp;'))
			.up()
			.up();
	}
